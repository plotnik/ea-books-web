...
- Use useState (imported from React) to manage input and submit state: e.g. [enteredPlayerName, setEnteredPlayerName] = useState('') and [submitted, setSubmitted] = useState(false).
- Add handleChange connected to input onChange; setEnteredPlayerName(event.target.value) and bind the input value to enteredPlayerName so it stays controlled.
- Add handleClick connected to button onClick; setSubmitted(true).
- Render the welcome text conditionally: if submitted is true show enteredPlayerName, otherwise show "unknown entity" (ternary inside JSX).
- Behavior: after clicking "Set Name" the entered name appears. If you keep typing afterwards the displayed name will change (because enteredPlayerName is reflected), unless you reset submitted to false inside handleChange to revert to "unknown entity" while editing.
- Conclusion: this works but requires managing keystroke updates and a second state flag — a case where using refs could simplify reading the input value at submit time.
