...
* _Recommendation:_ take the free LangGraph course at
https://academy.langchain.com/ for deeper learning.

*Basic concepts*

* _Graphs/workflows:_ composed of nodes (Python functions) and edges;
START and END are reserved nodes.
* _State:_ a workflow-wide context (think Python dict / TypedDict /
...
changes explicit.
* _Execution model:_ runs in discrete iterations called supersteps
...
* _Built-in/add reducers:_ use `typing.Annotated` with an operator (e.g.,
add) to concatenate lists.
* _Custom reducers:_ you can supply your own function to control how old
...
** Prompt engineering: tuning a prompt (or template) to improve
performance on a specific task.
** Prompt design: creating more general prompts that elicit better
responses across many tasks.
** Both build on in-context learning: LLMs can adapt to new tasks via
natural‑language instructions and examples.
* Key prompt abstractions in LangChain
** *PromptTemplate:* serializable runnable template that substitutes
variables at runtime.
** *ChatPromptTemplate / message templates:* build chat-style inputs
(system/human/assistant). You can pass messages as a list or as tuples
for convenience.
** *Placeholders / MessagesPlaceholder:* insert lists of messages at
runtime (common use: conversation history).
** *Partial templates and composition:* partially fill templates, add
functions as values, concatenate templates, or use
PipelinePromptTemplate to build larger prompts.
* Zero-shot vs few-shot prompting
** *Zero-shot:* task description without examples. Useful tricks: set a
role/persona and be explicit about style (creative, concise, factual).
** *Gemini guideline:* include persona, task, relevant context, and
desired format.
** *Few-shot:* include input–output examples to steer behavior. For large
example sets, load examples from files/db instead of hard-coding.
* Dynamic few-shot / example selection
** Select examples per input (semantic similarity, length/freshness
limits). LangChain provides built-in example selectors
(`langchain_core.example_selectors`) that can be plugged into
*FewShotPromptTemplate*.
* Chain-of-Thought (CoT)
** CoT prompts ask the model to show intermediate step-by-step reasoning
(e.g., _“Let’s think step by step”_), improving complex reasoning and math
performance.
** Often used zero-shot or with few-shot examples; many
reasoning-oriented models are trained in CoT format.
** Practical pattern: run a CoT chain, then parse/extract a concise
final answer from the CoT output.
* Self-consistency
** Sample multiple outputs at higher temperature, then aggregate (e.g.,
majority vote) to improve reliability for low-dimensional outputs like
classifications or extractions.
* Model/provider considerations
** Providers have different prompting recommendations (e.g., Anthropic
favors XML-like tags). Reasoning models (`o3-mini`, `gemini-flash-thinking`)
may have their own best practices and sometimes don’t need CoT/few-shot.
** When switching providers, re-evaluate prompts and end-to-end
performance; you may need multiple prompt templates and dynamic
selection by provider.
* Tools & references
** Papers: “The Prompt Report” (Schulhoff et al.) for prompt taxonomy;
CoT paper by Jason Wei et al.
** LangSmith Hub and LangChain utilities provide reusable prompt
artifacts and example prompt templates.
* Next concern
** If prompts grow too long and exceed a model’s context window,
additional strategies are needed (topic introduced as lead-in to the
next chapter).

[WARNING]
====
1. In the `MessagesPlaceholder` example, after invoking `chat_prompt_template.invoke` with `job_description="fake"` and `history=[("human", "hi!"), ("ai", "hi!")]`, how many messages are in the resulting messages list?

2. In the partial `PromptTemplate` example where `system_template.partial(a="a")` is invoked with `{"b": "b"}`, what exact text is printed?

3. What concise answer does the `final_chain.invoke` return for the question "Solve equation 2*x+5=15" in the Chain-of-Thought extraction example?
====
